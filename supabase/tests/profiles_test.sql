...
SELECT results_eq(
  ...
  ARRAY[0],
  'PROFILE-002: Authenticated user cannot see other profiles'
);

-- 11. PROFILE-002: authenticated user can see own profile
SELECT results_eq(
  'SELECT count(*)::int FROM profiles WHERE id = ''00000000-0000-0000-0000-000000000001''::uuid',
  ARRAY[1],
  'PROFILE-002: Authenticated user can see their own profile'
);
...
-- 13-14. PROFILE-001: Trigger auto-creates profile with full_name metadata
RESET role;
SELECT lives_ok(
  ...
  'PROFILE-001: Inserting into auth.users should succeed'
);

SELECT results_eq(
  $$SELECT display_name FROM profiles WHERE id = '00000000-0000-0000-0000-000000000099'$$,
  ARRAY['Test User'::text],
  'PROFILE-001: Trigger should create profile with display_name = full_name'
);

-- 15-16. PROFILE-001: Trigger fallback to 'name' metadata key
SELECT lives_ok(
  $$INSERT INTO auth.users (id, email, raw_user_meta_data, raw_app_meta_data, aud, role, instance_id, created_at, updated_at)
    VALUES (
  ...
  'PROFILE-001: Insert user with name metadata should succeed'
);

SELECT results_eq(
  $$SELECT full_name FROM profiles WHERE id = '00000000-0000-0000-0000-000000000098'$$,
  ARRAY['Jane Doe'::text],
  'PROFILE-001: Trigger should fallback to name metadata key'
);

-- 17-18. PROFILE-001: Trigger fallback to 'user_name' metadata key
SELECT lives_ok(
  $$INSERT INTO auth.users (id, email, raw_user_meta_data, raw_app_meta_data, aud, role, instance_id, created_at, updated_at)
    VALUES (
  ...
  'PROFILE-001: Insert user with user_name metadata should succeed'
);

SELECT results_eq(
  $$SELECT full_name FROM profiles WHERE id = '00000000-0000-0000-0000-000000000097'$$,
  ARRAY['janedoe'::text],
  'PROFILE-001: Trigger should fallback to user_name metadata key'
);

-- 19. PROFILE-003: Authenticated user can update own profile
SET local role authenticated;
SET local request.jwt.claims TO '{"sub": "00000000-0000-0000-0000-000000000001"}';

SELECT lives_ok(
  $$UPDATE profiles SET display_name = 'New Name' WHERE id = '00000000-0000-0000-0000-000000000001'$$,
  'PROFILE-003: Authenticated user can update own profile'
);

-- 20. PROFILE-003: Authenticated user cannot update other user's profile
SELECT results_eq(
  $$WITH updated AS (
  ...
  ARRAY[0],
  'PROFILE-003: Authenticated user cannot update other profiles'
);
