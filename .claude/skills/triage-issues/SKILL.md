...
2. **Verifiability** — can success be checked without subjective human judgment? Quantitative metrics, passing tests, or a precise visual spec all pass. "더 좋게", "더 깔끔하게", or unmetric'd "성능 개선" fail.
...
The split between `needs-clarification` and `needs-human` is whether *missing information* or *unmade decisions* drive the disqualification. "성능 개선" lacks a metric — one line from the author makes it verifiable. "사이트를 더 예쁘게" lacks a design — no single line resolves the taste decision.
...
- Product, UX, or design taste decisions ("더 예쁘게", "더 직관적으로", "make it nicer")
- Architectural or structural choice ("어떻게 구조화할지", "where should this live")
- Inventing a new abstraction or pattern not already present in the codebase

### 2. Outcome is not verifiable
- Subjective success criteria ("더 좋게", "더 깔끔하게", "improve UX")
