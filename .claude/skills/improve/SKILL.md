...
- **In this repo, read [references/claude-hunt.md](references/claude-hunt.md) now** — it carries the verified bun verification commands, the FSD / Supabase / UX-writing audit lenses, and the one workflow override (Linear is the single source of truth; `--issues` is disabled here in favor of `--linear`). Treat it as recon you don't have to redo.
...
- `--issues` (modifier on any planning invocation) → also publish each written plan as a GitHub issue via `gh`, URL recorded in the plan and index. Only with the explicit flag. **Before creating any issue, check whether the repo is public (`gh repo view --json visibility`). If it is, warn the user that issues are publicly visible and get explicit confirmation before publishing any plan that describes a security vulnerability, credential location, or other sensitive finding.** See [references/closing-the-loop.md](references/closing-the-loop.md). **In claude-hunt this flag is disabled** — Linear is the single intake gate; use `--linear` instead (see [references/claude-hunt.md](references/claude-hunt.md)).
- `--linear` (this repo's publish flag) → for each selected finding, route it through the **`capture-issue`** skill to create a Linear issue in the Claude Hunt team, recording the `CLA-N` identifier in the plan's Status block and the index. The `plans/` file stays the spec's source of truth; the Linear issue carries the work item and feeds the repo's `triage` → `dispatch-issue` loop. Never call `save_issue` directly. See [references/claude-hunt.md](references/claude-hunt.md).
