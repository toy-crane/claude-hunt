...
Claude Hunt를 최신 안정 Next.js로 올리고, 더 이상 유지되지 않거나 프로젝트에서 필요하지 않은 Next.js·Supabase 스킬을 제거한다. 이후 Next.js 관련 작업은 설치된 버전과 일치하는 공식 문서, 공식 에이전트 지침, 공식 런타임 검증 스킬만 사용한다.
...
- Next.js 및 Supabase 관련 스킬 목록에는 더 이상 사용하지 않기로 한 로컬·폐기 스킬이 노출되지 않는다.
...
- `supabase`와 `supabase-postgres-best-practices` 스킬을 모두 제거한다.
...
- `next-best-practices`, `nextjs`, `supabase`, `supabase-postgres-best-practices`가 더 이상 설치된 프로젝트 스킬로 발견되지 않는다.
...
- “공식 스킬만”은 이번 요청의 Next.js·Supabase 관련 스킬 정리를 뜻한다. 제품 개발 흐름과 디자인 등 다른 목적의 기존 스킬은 건드리지 않는다.
- Supabase 의존성과 애플리케이션 기능은 유지한다. 제거 대상은 에이전트 스킬뿐이다.
