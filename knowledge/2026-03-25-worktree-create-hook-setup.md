...
2. **Creates the worktree** at `.claude/worktrees/<name>` via `git worktree add`
3. **Copies gitignored `.env` files** by finding all `.env*` files (max depth 2), checking each with `git check-ignore`, and copying matches to the worktree
4. **Installs dependencies** with `bun install`
5. **Prints the worktree path** to stdout (required by Claude Code)

Key implementation details:
- `git worktree add` stdout is redirected to `/dev/null` to avoid polluting the path output
