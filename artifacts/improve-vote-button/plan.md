...
- **Acceptance**:
  - [ ] Applying `bg-vote` to an element produces the approved coral (`oklch(0.7 0.14 42)` light / `oklch(0.76 0.14 42)` dark) as its background.
  - [ ] `text-vote` applies the same coral as its foreground color.
  - [ ] No other element in the codebase references `--vote` outside the vote button (reserved accent).
- **Verification**:
...
- **Acceptance**:
  - [ ] Given `alreadyVoted=false`, `voteCount=128`, `isAuthenticated=true`, `ownedByViewer=false` → a single button with accessible name "추천하기", `aria-pressed="false"`, and visible text containing only "128" renders. The pill's idle coral styling is verified as a class-presence proxy (`border-vote` + `text-vote`) — swap for a computed-style check if the utility scheme changes.
  - [ ] Clicking that button synchronously shows "129" inside the button and sets `aria-pressed="true"` before any server round-trip resolves.
  - [ ] While the server call is in flight, the button carries the `disabled` attribute and a reduced-opacity class/state (class-presence proxy for the visible dim effect) and therefore cannot be clicked again.
  - [ ] When `toggleVote` resolves with `{ ok: true }`, the button remains with `aria-pressed="true"` and the visible count stays "129" (happy-path confirm).
  - [ ] When `toggleVote` resolves with `{ ok: false }`, the rendered count returns to "128", `aria-pressed` returns to `"false"`, and the button becomes interactive again (no `disabled` attribute).
- **Verification**:
...
- **Acceptance**:
  - [ ] Given `isAuthenticated=false` and `voteCount=42` → an element with link role, accessible name "추천하기", `href="/login"`, and visible text containing only "42" renders.
  - [ ] Clicking the link does not change the visible count (no optimistic path on the anonymous branch).
  - [ ] The link has no `disabled` attribute and is keyboard-activatable (focus visible, Enter activates).
- **Verification**:
...
- **Acceptance**:
  - [ ] Given `ownedByViewer=true`, `voteCount=7` → no element queryable by accessible name "추천하기" exists; the count "7" appears exactly once in the output.
  - [ ] The owner indicator does not use any of the coral utility classes (`bg-vote`, `text-vote`, `border-vote`); it uses `text-muted-foreground`.
  - [ ] The owner indicator is not focusable and carries no `aria-pressed` attribute.
- **Verification**:
...
- **Acceptance**:
  - [ ] A card with `vote_count=5` and a provided `renderVoteButton` shows the number "5" exactly once on the card.
  - [ ] The `renderVoteButton` slot is rendered at the top-right of the card's content area (to the right of the title/tagline/author block), not in a bottom action row.
  - [ ] When a `renderOwnerActions` is provided for the viewer-as-owner case, edit/delete still render below the card's text content (unchanged placement).
- **Verification**:
...
- [ ] End-to-end visual check on the running dev server: project board shows the new vertical coral pill for signed-in non-owners, a `/login` link-pill for anonymous viewers, and a muted read-only count for owners — with exactly one count per card. Evidence saved under `artifacts/improve-vote-button/evidence/`.
