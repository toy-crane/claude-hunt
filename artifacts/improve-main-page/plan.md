...
>
> **No wireframe.** The prior `wireframe.html` was stale (drew a compact mobile row inconsistent with the spec) and has been removed. The authoritative visual sources are `spec.md` plus `artifacts/improve-main-page/references/TerminalBoard.jsx` and `terminal-board.html`.
>
> **Desktop author cell.** The spec does not mandate dropping the avatar-initial bubble in the desktop AUTHOR column, so the plan keeps it. If a reviewer wants pixel-parity with `TerminalBoard.jsx` (nickname only), add a separate follow-up — it is a visual refinement, not a spec criterion.
...
- `Cohort` (from `@entities/cohort`) — already used by `ProjectBoard`; Task 2 threads a `cohortLabelsById` map derived from this list down into `ProjectCard` so the mobile meta line can render the class label next to the rank.
...
| `vercel-react-best-practices` | Tasks 1, 2 | `useCallback`/`useEffect` for the `popstate` listener; avoid re-subscribing on every render. |
| `frontend-ui-engineering` | Task 2 | Mobile stacked card layout; consistent spacing and typographic scale. |
| `fsd` | Task 2 | Row rendering belongs in `widgets/project-grid`; `ProjectCard` keeps ownership of both viewports. |
| `shadcn` | Tasks 1, 2 | Do not modify `shared/ui/*`; reuse existing primitives via class overrides. |
| `browser-testing-with-devtools` | Task 3 | Real-browser verification and evidence screenshots for each scenario. |

## Affected Files

| File Path | Change Type | Related Task |
|-----------|-------------|--------------|
| `app/_components/project-board.tsx` | Modify | Task 1 (pushState + popstate), Task 2 (pass `cohortLabelsById` into `ProjectGrid`) |
| `app/_components/project-board.test.tsx` | Modify | Task 1 |
| `widgets/project-grid/ui/project-grid.tsx` | Modify | Task 2 (thread `cohortLabelsById` to `ProjectCard`) |
| `widgets/project-grid/ui/project-grid.test.tsx` | Modify | Task 2 |
| `widgets/project-grid/ui/project-card.tsx` | Modify | Task 2 (rewrite `min-[720px]:hidden` branch) |
| `widgets/project-grid/ui/project-card.test.tsx` | Modify | Task 2 |
| `widgets/project-grid/index.ts` | Modify | Task 2 (re-export updated `ProjectGridProps` with `cohortLabelsById`) |
| `features/cohort-filter/ui/cohort-chips.tsx` | Modify | Task 2 (swap `flex-wrap` for mobile `flex-nowrap overflow-x-auto`) |
| `features/cohort-filter/ui/cohort-chips.test.tsx` | Modify | Task 2 |
| `artifacts/improve-main-page/evidence/` | New | Task 3 (screenshot evidence + `verification.md`) |
...
### Task 2: Mobile stacked card + horizontal-scroll filter chips

- **Covers**: Scenario 7 (full); Scenario 2 chip-overflow criterion (partial — just the overflow-scroll bullet; the other 7 chip criteria already pass)
- **Size**: M (7 files)
- **Dependencies**: None
- **References**:
  - `test-driven-development`
  - `frontend-ui-engineering` — stacked card spacing, type scale
  - `fsd` — row layout stays in `widgets/project-grid`; chip layout in `features/cohort-filter`
...
- **Implementation targets**:
  ...
  - `features/cohort-filter/ui/cohort-chips.tsx` — replace the root `flex flex-wrap` with `flex flex-nowrap overflow-x-auto min-[720px]:flex-wrap min-[720px]:overflow-visible`; ensure the active chip's inverted-color state is visible regardless of viewport.
  - `features/cohort-filter/ui/cohort-chips.test.tsx` — assert the container exposes horizontal-scroll overflow below 720 px and wraps at ≥ 720 px.
- **Acceptance**:
  ...
  - [ ] The meta line includes the project's class label after the rank, separated by a `·` divider (class label resolved via `cohortLabelsById`); when the project has no `cohort_id`, the `· {label}` segment is omitted.
  ...
  - [ ] At viewport widths ≥ 720 px, the desktop grid row continues to render exactly as before this task (table header visible, row columns unchanged).
  - [ ] At viewport widths < 720 px, when the chip row total width exceeds the viewport, the chips become horizontally scrollable (no wrapping onto a second line).
  - [ ] At viewport widths ≥ 720 px, chips continue to wrap onto a second line when necessary (no horizontal scroll).
- **Verification**:
  - `bun run test:unit -- widgets/project-grid`
  - `bun run test:unit -- features/cohort-filter/ui/cohort-chips`
  - `bun run test:unit -- app/_components/project-board`
  - `bun run build`
  - Browser MCP — resize to 375 × 812, load `/`, capture the first two rows; save to `artifacts/improve-main-page/evidence/task-2-mobile-stacked.png`.
  - Browser MCP — at 375 × 812, scroll the chip row horizontally and capture the start + end positions; save to `artifacts/improve-main-page/evidence/task-2-mobile-chip-scroll.png`.
  - Browser MCP — resize to 1024 × 768, load `/`, capture the table header + first row (including an owner-action row if the viewer owns one); save to `artifacts/improve-main-page/evidence/task-2-desktop-regression.png`.
...
### Task 3: Evidence walk — verify every spec scenario in a real browser
...
- **Acceptance (each bullet is a separate row in `verification.md`)**:
  ...
  - [ ] **Scenario 7 (mobile).** All five Success Criteria (stacked order, meta line, 16:10 thumb, typography, bottom row, chip horizontal scroll) are proved by `widgets/project-grid/ui/project-card.test.tsx` + `features/cohort-filter/ui/cohort-chips.test.tsx` (after Task 2) plus a 375 × 812 screenshot.
...
- **Mobile meta line when `cohort_id` is null.** The sample data always has a cohort, but the production payload may carry rows with `cohort_id = null` (e.g. a legacy row). The spec does not say what the meta line should show in that case. Default proposal in Task 2: render only the rank segment, omitting the `·` divider and class label. Flag for the reviewer; adjust if the answer is different.
