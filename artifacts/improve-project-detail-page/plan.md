...
| `screenshot_path` column | **Make nullable in this scope**; the actual DROP and the `images` length CHECK ship in a follow-up scope (Expand-Contract migration pattern). View exposes `primary_image_path = coalesce(images[0]->>'path', screenshot_path)` for the entire transition window. | Avoids breaking old code during the deploy window. Once new code is fully live and stable, a follow-up PR contracts the schema (drops the column, drops the coalesce shim, adds the 1..5 CHECK). |
...
- **Implementation targets** (Expand-only — additive + nullable; no length CHECK, no drops):
  - `supabase/schemas/projects.sql` — add `images jsonb not null default '[]'` (no length CHECK); add `github_url text null check (github_url ~ '^https://github\.com/[^/\s]+/[^/\s]+/?$')`; **change `screenshot_path` from `not null` to nullable** so that after T7 new rows can be written without it.
  - `supabase/schemas/projects_with_vote_count.sql` — add `primary_image_path` derived as `coalesce(images->0->>'path', screenshot_path)` (handles both old and new rows); also project `github_url`. Keep `screenshot_path` projected for one cycle.
  - `supabase/migrations/<ts>_projects_multi_image_github.sql` — generated by `db diff`; review per checklist
  - `supabase/migrations/<ts>_backfill_project_images.sql` — manual one-time DML: `UPDATE public.projects SET images = jsonb_build_array(jsonb_build_object('path', screenshot_path)) WHERE jsonb_array_length(images) = 0 AND screenshot_path IS NOT NULL;`
...
- **Acceptance**:
  ...
  - [ ] `images` column accepts `'[]'` at this point (no length CHECK in this scope; CHECK ships in the follow-up Contract scope)
  - [ ] `screenshot_path` accepts NULL on new inserts (NOT NULL constraint dropped)
  ...
  - [ ] Existing single-screenshot writes (current `submitProject` / `editProject`) still succeed (both columns still writable)
- **Verification**:
...
- **Implementation targets**:
  ...
  - `features/submit-project/api/schema.ts` — replace `screenshotPath: string` with `imagePaths: string[].min(1).max(5)` (validation enforced at the application layer for now; the DB CHECK lands in the follow-up Contract scope)
  - `features/submit-project/api/actions.ts` — insert `images = imagePaths.map(p => ({path: p}))`; **stop writing `screenshot_path`** (T1 made the column nullable, so new rows leave it NULL; the view's `coalesce` still serves old rows correctly)
  ...
  - **No schema changes in this task.** Length CHECK and column drop are deferred to the follow-up Contract scope (see "Follow-up Cleanup" section).
- **Acceptance**:
  ...
  - [ ] New rows inserted by the updated action have `screenshot_path = NULL` and `images` populated; old rows still read correctly through the view's `coalesce`
- **Verification**:
...
## Follow-up Cleanup (separate scope, after this feature ships and stabilizes)

This is the **Contract** half of the Expand-Contract migration. Do NOT include in this PR. Open a follow-up after the new code has been live and writing `images` exclusively for at least one week (or one full deploy cycle), and observability confirms no rows are being inserted with empty `images`.

- [ ] `supabase/schemas/projects.sql` — add `check (jsonb_array_length(images) between 1 and 5)`; drop `screenshot_path` column
...
- [ ] Generate migration via `db diff` and review per the checklist
- [ ] Add pgTAP assertions: `images = '[]'` is rejected; `images` with 6 entries is rejected; `screenshot_path` column no longer exists
- [ ] Pre-flight: query production for any rows with `jsonb_array_length(images) = 0` and patch them before applying the CHECK
